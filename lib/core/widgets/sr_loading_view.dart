...
/// The 2px indeterminate route-progress bar (§ 3.10), an indigo→violet gradient
/// pinned under the app bar.
///
/// Under reduced motion it becomes a **static full-width bar at 85% opacity**
/// rather than disappearing — the web is explicit that feedback is preserved and
/// only the movement is dropped.
class SrRouteProgressBar extends StatefulWidget {
  const SrRouteProgressBar({super.key});

  @override
  State<SrRouteProgressBar> createState() => _SrRouteProgressBarState();
}

class _SrRouteProgressBarState extends State<SrRouteProgressBar>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controller = AnimationController(
    vsync: this,
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      ...
      child: SizedBox(
        height: 2,
        child: !SrMotion.respects(context)
            ? Opacity(
                opacity: 0.85,
                child: DecoratedBox(
